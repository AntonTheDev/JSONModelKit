...
class Serializer:
   ...
   def serializerString(self):
      ...
      fileString = str.replace(fileString, "{ RETURN_ALL }",                        self.serialization_full_return(propertyMappings, uniqueGroups))
   ...
   '''
   Replaces { RETURN_ALL } in the template
   '''

   def serialization_full_return(self, propertyMappings, uniqueGroups):
      runningClassString = ""
      ...
      for propertyKey in propertyMappings.keys():
         ...
         propertyJSONKey = str(propertyMap[MappingKey.Key])
         optional = self.is_property_mapping_optional(propertyMap)
            
         if propertyType in Type.NativeTypes:
            runningClassString += "\t\tparams[\"" + propertyJSONKey 
            runningClassString += "\"] = " + propertyKey + "\r\n" 
         elif propertyType in Type.CollectionTypes:
            runningClassString += self.collectionString(propertyKey, propertyMap, "X", optional)
         else:
            if optional == False:
               runningClassString += "\t\t\tparams[\"" + propertyJSONKey + "\"] = " + propertyKey + ".params(forGroup :\"\")\r\n\t\t\r\n" 
            else:
               runningClassString += "\t\tif let instance = " + propertyKey + " { \r\n"
               runningClassString += "\t\t\tparams[\"" + propertyJSONKey + "\"] =  instance.params(forGroup :\"\")\r\n\t\t}\r\n\r\n" 
